...
class DropList {
    ...
    toggleFocusedItem() {
        ...
        if (this.hasFocusedItem() && p.multi) {
            ...
            this._trigger('check', { value: item.value, item: item[ItemSymbol] ?? item });
        ...
        if (p.multi && p.autoCheckGroupChildren) {
            ...
            if (item._group) {
                ...
                for (let i = groupIndex + 1, len = items.length; i < len; i++) {
                    ...
                    // Fire event
                    this._trigger('check', { value: next.value, item: next[ItemSymbol] ?? next, isCheckingGroup: true });
                }
    ...
    _updateGroupCheckedState(groupIndex, fireEvents) {
        ...
        if (p.multi && p.autoCheckGroupChildren && groupIndex > -1) {
            ...
            if (!!groupItem._checked !== shouldCheckGroup) {
                ...
                if (fireEvents) {
                    // Fire event
                    this._trigger('check', { value: groupItem.value, item: groupItem[ItemSymbol] ?? groupItem });
                }
            }
        }
