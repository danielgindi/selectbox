...
class DropList {
    ...
    blurFocusedItem(fromFocus) {
        ...
        let active = p.active;
        active.classList.remove(`${p.baseClassName}__item_focus`);
        p.active = null;

        const item = active[ItemSymbol];
        if (!item)
            return;
    ...
    toggleFocusedItem() {
        ...
        if (p.active && p.multi) {
            let item = p.active[ItemSymbol];
            if (item._nocheck || item._nointeraction) return this;

            item._checked = !item._checked;
            toggleClass(p.active, `${p.baseClassName}__item_checked`, item._checked);
    ...
    triggerItemSelection(event) {
        ...
        p.active = p.active || closestUntil(event.target, 'li', p.el);
        let item = p.active[ItemSymbol];
        ...
        if (!p.multi) {
            this._setSingleSelectedItemEl(p.active);
        }
    ...
    setFocusedItemAtIndex(itemIndex) {
        ...
        if (p.active) {
            p.active.classList.remove(`${p.baseClassName}__item_focus`);
            p.active = null;
        }

        let item = p.items[itemIndex];
        ...
        if (itemIndex > -1) {
            ...
            let itemElement = p.virtualListHelper.getItemElementAt(itemIndex);
            if (itemElement) {
                itemElement.classList.add(`${p.baseClassName}__item_focus`);
                p.active = itemElement;
    ...
    _setSingleSelectedItemEl(itemEl) {
        ...
        if (p.singleSelected) {
            p.singleSelected.classList.remove(`${p.baseClassName}__item_checked`);
            p.singleSelected = null;
        }

        if (itemEl) {
            itemEl.classList.add(`${p.baseClassName}__item_checked`);
            p.singleSelected = itemEl;
        }
    ...
    isLastItem() {
        ...
        return !!p.active
            && p.active[ItemSymbol] === p.items[p.items.length - 1];
    }
    ...
    _handleMouseOver(event, itemEl) {
        ...
        for (let node of itemEl.parentNode.childNodes)
            if (node instanceof Element)
                node.classList.remove(`${p.baseClassName}__item-active`);
    ...
    _hookFocusEvents() {
        ...
        p.sink
            .add(p.el, 'focus', (event, keepActiveItem) => {
                let itemEl = p.active || // active item
    ...
    _keydownFreeType(evt) {
        ...
        let activeIndex = p.active ? p.items.indexOf(p.active[ItemSymbol]) : -1;
        let matchIndex = -1;
    ...
    _focus(event, itemEl, itemIndex) {
        ...
        this._scrollIntoView(itemEl, itemIndex);

        let active = itemEl || p.virtualListHelper.getItemElementAt(itemIndex);

        if (active) {
            if (active[ItemSymbol]._nointeraction) {
                if (p.active)
                    this.blurFocusedItem();
            } else {
                active.classList.add(`${p.baseClassName}__item_focus`);
                p.active = active;
    ...
    _scrollIntoView(itemEl, itemIndex) {
        const p = this._p;

        if (this._hasScroll()) {
            const el = p.el, scrollTop = el.scrollTop;

            let itemPos = p.virtualListHelper.getItemPosition(itemIndex);
    ...
    _move(direction, event) {
        ...
        if (direction === 'first') {
            ...
            nextIndex = p.items.indexOf(p.active[ItemSymbol]) - 1;
            if (nextIndex === -1) {
                nextIndex = p.items.length - 1;
            }
            ...
            if (this._hasScroll()) {
                ...
                if (p.virtualListHelper.isVirtual()) {
                    ...
                    nextIndex = p.items.indexOf(p.active[ItemSymbol]);
                    ...
                    let base = getElementOffset(p.active).top;
                    let height = getElementHeight(p.el, true);

                    while (true) { // eslint-disable-line no-constant-condition
                        next = p.active.nextElementSibling;
                        if (!next) return;
                    ...
                    if (next) {
                        nextIndex = p.items.indexOf(next[ItemSymbol]);
                    }
                }
    ...
    _updateWidth(positionOptions) {
        ...
        const elComputedStyle = getComputedStyle(el);
        let scrollWidth = el.scrollWidth + (parseFloat(elComputedStyle.borderLeftWidth) || 0) + (parseFloat(elComputedStyle.borderRightWidth) || 0);
        if (scrollWidth > newOuterWidth) {
            // consider scrollWidth delta
            newOuterWidth += el.scrollWidth - parseFloat(el.style.width);
