import { getCssProps, getElementWidth, getPseudoElementWidth, setCssProps } from '@danielgindi/dom-utils/lib/Css';
...
class SelectBox {
    ...
    _resizeInput() {
        ...
        if (p.multi && p.multiItemEls.length === 0 && p.showSelection) {
            ...
            // Measure these
            const minWidth = (parseFloat(getComputedStyle(input)['font-size']) || 0) * 0.75;
            const backBufferWidth = getElementWidth(backBufferEl);
            const currentWidth = getElementWidth(input);
            ...
            if (newWidth !== currentWidth) {
                ...
                if (input.scrollWidth > newWidth || input.scrollWidth > getElementWidth(input, true, false)) {
                    // consider scrollWidth delta
                    newWidth += input.scrollWidth - parseFloat(input.style.width);
                    input.style.width = `${newWidth}px`;
                }
            }
